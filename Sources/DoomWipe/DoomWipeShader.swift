...
public struct DoomWipeShader {
	...
	private static func getOffset(seed: Int) -> Data {
		...
		let seed = seed % 0xFF
		offsetsLookup[seed] = offsetsLookup[seed] ?? createPattern(seed: seed)
...
private func createPattern(seed: Int = 0) -> Data {
	func random(_ index: Int, _ seed: Int) -> Int {
		return rndtable[(index + seed + 1) & 0xFF]
	}
	...
	ints[0] = -(random(0, seed) % 16)

	for x in 1 ..< ints.count {
		let r = (random(x, seed) % 3) - 1
